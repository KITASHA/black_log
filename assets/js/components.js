(() => {
    ...
    const path = inSelenoa ? "../components/footer.html" : "components/footer.html";
    ...
    fetch(path)
        ...
        .then((html) => {
            ...
            if (brand && inSelenoa) brand.textContent = "SELENOA TECHNOLOGIES";
        })
        .catch((error) => {
            console.error("Shared footer could not be loaded.", error);
            mount.innerHTML = '<footer class="site-footer"><div class="site-footer__inner"><p class="site-footer__notice">このページはフィクションです</p></div></footer>';
        });
})();
